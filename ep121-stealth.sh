...
if [ "$B" = f ]; then
    ...
    sudo cp ${BINDIR}Default ${INIT}Default |sed ' /^exec florence &/ i\exec python /var/lib/gdm/ep121_drv.py &' ${INIT}Default > ${BINDIR}Default
    rm ${BINDIR}Default
    ...
    sudo cp ${BINDIR}Default ${INIT}Default |sed ' /^exec onboard &/ i\exec python /var/lib/gdm/ep121_drv.py &' ${INIT}Default > ${BINDIR}Default
    rm ${BINDIR}Default
    sudo ln -s ${HOMDIR}/.bin/ep121/ep121_drv.py ${GDM}
